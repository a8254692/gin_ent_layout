...
func (tx *Tx) init() {
	tx.MerchantChannelBind = NewMerchantChannelBindClient(tx.config)
	tx.MerchantList = NewMerchantListClient(tx.config)
	tx.SysMenu = NewSysMenuClient(tx.config)
	tx.SysPermissions = NewSysPermissionsClient(tx.config)
	tx.SysRoleMenu = NewSysRoleMenuClient(tx.config)
	tx.SysRolePermissions = NewSysRolePermissionsClient(tx.config)
	tx.SysRoleUsers = NewSysRoleUsersClient(tx.config)
	tx.SysRoles = NewSysRolesClient(tx.config)
...
// applies a query, for example: MerchantChannelBind.QueryXXX(), the query will be executed
